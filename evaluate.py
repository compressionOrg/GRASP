...
def main_test(model_name: str, device: str, compression_ratio: float):
    import gsvd
    gsvd_model = gsvd.compress(
        ...
        merge=False,
        verbose=False,
        save_path="./checkpoint/llama2_7b_winogrande_0.2.pth"
    )
    result = evaluate_model(gsvd_model.model, tokenizer, model_name=model_name, tasks="hellaswag", eval_ppl="", device=device)
...
if __name__ == "__main__":
    ...
    calibration_dataloader = get_calibration_dataloader(dataset_name="hellaswag", tokenizer=tokenizer, num_samples=512, batch_size=1, seq_len=512)

    main_test(model_name="llama", device="cuda:1", compression_ratio=0.2)